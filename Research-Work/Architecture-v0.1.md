...
![Wheel of Emotions by Robert Plutchik](imgs\WheelOfEmotions.webp)
...
### Parts of the **JAST Approach**

- **NEP** or **_Natural Emotion Processing_**: This project defines **Natural Emotion Processing (NEP)** as a specialized form of data processing that goes beyond standard linguistic analysis to interpret the emotional content of human communication. Unlike traditional _Natural Language Processing (NLP)_ which focuses on syntax and semantics, NEP aims to understand the nuanced emotional state embedded within a given input, regardless of its modality (text, audio, or visual).

- **E-Tokens** or **_Emotional Tokens_**: An **E-Token** serves as a structured, intermediate data representation of an input's emotional state. These tokens are generated by a series of specialized encoders that extract emotionally relevant features from raw data. The _E-Token_ is designed to be a standardized, machine-readable summary of the emotional signal, regardless of whether it originates from _text_, _audio_, or _visual_ input.

- **Format of E-Tokens**: The _E-Token_ format is structured to provide a concise yet comprehensive summary of the emotional input. Each token is formatted as <br>`[dominant-emotional-dimension][intensity][mode-of-input]`

  - **Dominant Emotional Dimension**: This is represented by a single or dual-character code, using uppercase letters (`J`, `A`, `S`, `T`) for the four dominant emotions (_Joy_, _Anger_, _Surprise_, _Trust_) and lowercase letters (`j`, `a`, `s`, `t`) for their respective opposites (_Sadness_, _Fear_, _Anticipation_, _Disgust_). A combination, such as `Js`, can be used for mixed emotions like _optimism_.
  - **Intensity**: This is a numerical value (integer or float) that quantifies the strength of the expressed emotion.
  - **Mode of Input**: This indicates the source modality of the data (`t` for _text_, `a` for _audio_, `v` for _video_).

- **ExsDat**: **Excess Data (ExsDat)** is a critical component of the multimodal processing pipeline. It refers to a supplementary data stream that captures and quantifies emotional cues that are not present in the primary input modality. For example, in the case of a video with speech, the _text_ is the primary input, and the _facial expressions_ and _vocal tone_ are captured as _ExsDat_. The JAST Algorithm uses _ExsDat_ to adjust the final emotional vector, enabling it to correctly interpret nuanced human expressions like sarcasm.

- **JAST-V Points** or **_JAST Vector Points_**: A **JAST-V Point** is the final, _four-dimensional_ vector output of the **JAST Algorithm**. This vector plots the emotional state of a given input within the continuous, multi-dimensional emotional space. Each of the four vector components corresponds to an axis on the bipolar emotional scale (**Joy/Sadness**, **Anger/Fear**, **Surprise/Anticipation**, and **Trust/Disgust**). Unlike the *E-Token*, which is a summarized representation, the *JAST-V Point* provides the precise coordinates for plotting and analyzing the data in the emotional *octant*.
